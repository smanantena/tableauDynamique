...
class AllProductsSortedByNameLimit
{
    ...
    public static function execute (int $limit = 0, string $order = 'asc') : ?array
    {
        try {
            ...
            if (strtoupper($order) === 'DESC') {
                if ($limit > 0) {
                    $query = $PDO->prepare('SELECT * FROM products ORDER BY name DESC LIMIT :limit;');
                    
                    $query->execute([
                        "limit" => $limit
                    ]);
                } else {
                    $query = $PDO->query('SELECT * FROM products ORDER BY name DESC;');
                    
                }
            } else {
                if ($limit > 0) {
                    $query = $PDO->prepare('SELECT * FROM products ORDER BY name ASC LIMIT :limit;');
                    $query->execute([
                        "limit" => $limit
                    ]);
                } else {
